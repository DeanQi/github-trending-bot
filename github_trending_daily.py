...
每天抓取 GitHub Trending 日榜，筛选高星/快速增长项目，生成 Markdown 日报并发送到飞书。
    ...
    for i, repo in enumerate(repos):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if remaining == "0":
                    print(f"  [API] Rate limit reached, stopping at repo {i+1}/{len(repos)}")
                    break
            elif resp.status_code == 404:
                pass
        except Exception as e:
            print(f"  [API] Error fetching {repo['owner']}/{repo['name']}: {e}")
        time.sleep(0.15)
    return repos
...
def generate_md_report(repos, high_star, fast_grow):
    ...
    lines.append("")
    lines.append(f"> 共收录 {len(repos)} 个 Trending 项目 | 生成时间 {now.strftime('%H:%M')} CST")
    lines.append("")
    ...
    if high_star:
        for i, r in enumerate(high_star, 1):
            ...
            archived = " `[归档]`" if r.get("archived") else ""
            lines.append(f"**{i}. [{r['owner']}/{r['name']}]({url})**{archived}")
            lines.append(f"   {desc}")
            lines.append(f"   `{lang}` | ⭐ {r.get('total_stars', 0):,} | 🍴 {r.get('forks', 0):,} | 📈 +{r.get('stars_today', 0):,} today")
            lines.append("")
    else:
    ...
    if fast_grow:
        for i, r in enumerate(fast_grow, 1):
            ...
            lines.append(f"**{i}. [{r['owner']}/{r['name']}]({url})**{archived}")
            lines.append(f"   {desc}")
            lines.append(f"   `{lang}` | ⭐ {r.get('total_stars', 0):,} | 📈 +{r.get('stars_today', 0):,} today")
            lines.append("")
    else:
