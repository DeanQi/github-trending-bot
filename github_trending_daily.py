...
if GITHUB_TOKEN:
    API_HEADERS[Authorization] = fBearer {GITHUB_TOKEN}


def fetch_trending_page(since=daily):
    "抓取 GitHub Trending 页面原始 HTML"
    url = fhttps://github.com/trending?since={since}
...
def enrich_with_api(repos):
 ...
 "
    for i, repo in enumerate(repos):
        url = fhttps://api.github.com/repos/{repo['owner']}/{repo['name']}
        try:
...
def generate_md_report(repos, high_star, fast_grow):
    ...
    lines.append(")
    lines.append(f共 {len(fast_grow)} 个)
    lines.append(")
    if fast_grow:
...
def main():
    ...
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    report_path = os.path.join(OUTPUT_DIR, fgithub_trending_{datetime.now().strftime('%Y%m%d')}.md)
